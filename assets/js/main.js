(function ($) {
  ...
  /*===================
    ...
    $(".navbar-nav a[href^='#'], .scroll").click(function () {
        isNavClicked = true;
    ...
    function growRecords() {
        $('#records .stat').each(function () {
            ...
            looper(total, unit,  container, 1200);
        });
    }

    function looper(total, unit, target, duration) {
        if (duration) {
            var counter = 1;
            var speed = parseInt(duration / total);
            var interval = setInterval(function () {
                if (counter <= total) {
                    target.html(counter.toString() +unit);
                } else {
                    target.html(total.toString()+unit);
